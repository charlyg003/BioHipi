...
public class HipiImageBundle {
			...
			try {
				...
				if (decoder == null) {
					throw new IOException(
							"Unsupported storage format in image record ending at byte offset: " + currentOffset);
				}
				...
				if (culler != null) {
					if (culler.includeExifDataInHeader()) {
						...
						//		imageHeader.setValue(HipiImageHeader.INDEX_JPEG_PNG_EXIF_DATA, imageHeaderWithExifData.getAllExifData());
				...
				case BYTE:
					...
					try {
						...
						System.err.println(
								"Runtime exception while attempting to decode raster image: " + e.getMessage());
						e.printStackTrace();
				...
				case RAW:
					try {
						...
						System.err.println("Runtime exception while attempting to create RawImage: " + e.getMessage());
						e.printStackTrace();
						// Attempt to keep going
						return nextKeyValue();
					}
	...
	public void addImage(HipiImageHeader imageHeader, InputStream imageStream) throws IOException {
		...
		if (fileMode != FILE_MODE_WRITE) {
			throw new IOException("HIB [" + indexFilePath.getName()
			+ "] is not opened for writing. Must successfully open HIB for writing before calling this method.");
		}
