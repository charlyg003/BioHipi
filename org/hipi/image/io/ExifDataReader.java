...
/**
 * Routines for extracting EXIF data contained in a JPEG or PNG image and for
 * flattening this type of data to produce a single key/value dictionary.
 */
public class ExifDataReader {
	...
	public Metadata extract() throws ImageProcessingException, IOException {
		return ImageMetadataReader.readMetadata(ubis, false);
	}
	...
	public static HashMap<String, String> extractAndFlatten(InputStream is) throws IOException {
		...
		try {
			...
			throw new IOException(String.format("Encountered error while attempting to extract image EXIF data [%s]",
					ex.getMessage()));
		}
		return exifData;
	}
