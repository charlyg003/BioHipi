...
public class HibToJpegFromDicom extends Configured implements Tool {

  public static class HibToDicomMapper extends Mapper<HipiImageHeader, DicomImage, BooleanWritable, Text> {
    ...
    public void setup(Context context) throws IOException {
      ...
      path = new Path(conf.get("dicomtojpegfromhib.outdir"));
     ...
     */
    @Override
    public void map(HipiImageHeader header, DicomImage image, Context context) throws IOException, InterruptedException {
      ...
      Path outpath = new Path(path + "/" + base + ".jpeg");

      // Write image file to HDFS
      FSDataOutputStream os = fileSystem.create(outpath);
      JpegCodec.getInstance().encodeImage(image.getFloatJpegImage(), os);
      os.flush();
      os.close();
  ...
  public int run(String[] args) throws Exception {
    ...
    // Check arguments
    if (args.length != 2) {
      System.out.println("Usage: hibToJpegFromDicom.jar <input HIB> <output directory>");
      System.exit(0);
    }
    ...
    conf.setStrings("dicomtojpegfromhib.outdir", outputPath);

    // Setup MapReduce classes
    Job job = Job.getInstance(conf, "dicomtojpegfromhib");
    job.setJarByClass(HibToJpegFromDicom.class);
    job.setMapperClass(HibToDicomMapper.class);
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new HibToJpegFromDicom(), args);
    System.exit(res);
  }
