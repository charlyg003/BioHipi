...
public class HibToPngFromDicom extends Configured implements Tool {
  ...
  public static class HibToDicomMapper extends Mapper<HipiImageHeader, DicomImage, BooleanWritable, Text> {
    ...
    public void setup(Context context) throws IOException {
      ...
      path = new Path(conf.get("dicomtopngfromhib.outdir"));
    ...
    public void map(HipiImageHeader header, DicomImage image, Context context) throws IOException, InterruptedException {
      ...
      Path outpath = new Path(path + "/" + base + ".png");

      // Write image file to HDFS
      FSDataOutputStream os = fileSystem.create(outpath);
      PngCodec.getInstance().encodeImage(image.getFloatPngImage(), os);
      os.flush();
      os.close();
  ...
  public int run(String[] args) throws Exception {
    ...
    // Check arguments
    if (args.length != 2) {
      System.out.println("Usage: hibToPngFromDicom.jar <input HIB> <output directory>");
      System.exit(0);
    }
    ...
    conf.setStrings("dicomtopngfromhib.outdir", outputPath);

    // Setup MapReduce classes
    Job job = Job.getInstance(conf, "dicomtopngfromhib");
    job.setJarByClass(HibToPngFromDicom.class);
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new HibToPngFromDicom(), args);
    System.exit(res);
  }
