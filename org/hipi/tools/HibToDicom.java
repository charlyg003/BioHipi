...
public class HibToDicom extends Configured implements Tool {
  ...
  public static class HibToDicomMapper extends Mapper<HipiImageHeader, DicomImage, BooleanWritable, Text> {
    ...
    public void setup(Context context) throws IOException {
      ...
      path = new Path(conf.get("dicomfromhib.outdir"));
    ...
    public void map(HipiImageHeader header, DicomImage image, Context context) throws IOException, InterruptedException {
      ...
      DicomCodec.getInstance().encodeImage(image, os);
      os.flush();
      os.close();
  ...
  public int run(String[] args) throws Exception {
    ...
    // Check arguments
    if (args.length != 2) {
      System.out.println("Usage: hibToDicom.jar <input HIB> <output directory>");
      System.exit(0);
    }
    ...
    conf.setStrings("dicomfromhib.outdir", outputPath);

    // Setup MapReduce classes
    Job job = Job.getInstance(conf, "dicomfromhib");
    job.setJarByClass(HibToDicom.class);
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new HibToDicom(), args);
    System.exit(res);
  }
