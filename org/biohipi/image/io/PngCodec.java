...
//import org.biohipi.image.PixelArray;
 ...
 * {@link ImageEncoder} for the PNG image storage format. Currently only
 * supports RGB encodings.
 */
public class PngCodec extends RasterCodec {
	...
	/**
	 * Decodes the image header from an input stream that contains the PNG
	 * image. PNG images are broken up into "chunks" (see PNG documentation),
	 * and the PNG header could be located anywhere in the image
	 * 
	 * @param inputStream
	 *            the {@link InputStream} that contains the PNG image
	 * @return {@link BioHipiImageHeader} found in the input stream
	 */
	public BioHipiImageHeader decodeHeader(InputStream inputStream) throws IOException {
		...
		while (trucking) {
			try {
				...
				if (typeString.equals("IHDR")) {
					...
					dis.readFully(data);
					// Read the CRC.
					long crc = dis.readInt() & 0x00000000ffffffffL; // Make it
					// unsigned.
		...
		if (width <= 0 || height <= 0) {
			throw new IOException(
					"Failed to decode PNG image header. (Found invalid dimensions width <= 0 or height <= 0.)");
		}

		BioHipiImageHeader header = new BioHipiImageHeader(BioHipiImageFormat.PNG);
	...
	class PNGData {
		...
		public byte[] getImageData() {
			try {
				...
				int length = width * height * bitsPerPixel / 8 * 3; // hard code
				// the 3 for
				// RGB for
				// now
				...
				if (getInterlace() == 0) {
					...
					for (int i = 0; i < length; i++) {
						if (i % (width * bitsPerPixel / 8 * 3) == 0) { // again,
							// hard
							// code
							// the 3
							// for
	 ...
	 * 
	 * @param image
	 *            the input {@link BioHipiImage} to be encoded
	 * @param os
	 *            the {@link OutputStream} that the encoded image will be
	 *            written to
	 */
	public void encodeImage(BioHipiImage image, OutputStream os) throws IllegalArgumentException, IOException {
		...
		crc = new CRC32();
		int width = ((RasterImage) image).getWidth();
		int height = ((RasterImage) image).getHeight();
		...
		case COLOR_MODE:
			for (int y = 0; y < height; y++) {
				...
				for (int x = 0; x < width; x++) {
					/*
					 * int r = Math.min(Math.max((int) (image.getPixel(x, y, 0)
					 * * 255), 0), 255); int g = Math.min(Math.max((int)
					 * (image.getPixel(x, y, 1) * 255), 0), 255); int b =
					 * Math.min(Math.max((int) (image.getPixel(x, y, 2) * 255),
					 * 0), 255);
					 */
					int r = ((RasterImage) image).getIntElem((y * width + x) * 3 + 0);
					int g = ((RasterImage) image).getIntElem((y * width + x) * 3 + 1);
					int b = ((RasterImage) image).getIntElem((y * width + x) * 3 + 2);
					bos.write((byte) r);
					bos.write((byte) g);
					bos.write((byte) b);
				}
